...
import {
  User, Mail, Lock, Phone, CheckCircle, ArrowRight, ArrowLeft,
  UploadCloud, Award,
} from "lucide-react";
...
export default function RegisterAdvokatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!licenseFile)  { alert("Harap upload Kartu Tanda Pengenal Advokat terlebih dahulu."); return; }
    setIsLoading(true);
    ...
    try {
      ...
      document.cookie = `role=lawyer; path=/; max-age=3600; SameSite=Strict`;
      ...
      // Step 4: Upload Lisensi Advokat
      setStatusMsg("Mengupload Lisensi Advokat...");
      const licForm = new FormData();
      licForm.append("license_file", licenseFile);
      await apiFetch(`/users/${userId}/upload-license`, { method: "POST", body: licForm });
  ...
  const FileDropZone = ({
    id, label, file, setFile, icon: Icon = UploadCloud, hint,
  }: {
    ...
    <div>
      ...
      <div className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-md transition-colors bg-slate-50 cursor-pointer ${
        ...
        <div className="space-y-1 text-center">
          <Icon className={`mx-auto h-12 w-12 ${file ? "text-green-500" : "text-blue-500"}`} />
  ...
  return (
    <div className="w-full">
      <div className="mb-8">
        ...
        <p className="mt-2 text-base text-slate-600">Mari berkontribusi memberikan akses keadilan bagi masyarakat kurang mampu.</p>
      </div>
            ...
            {currentStep === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div>
                  ...
                  <div className="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleChange} required
                      className="focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 sm:text-sm border-slate-300 rounded-md py-3 border outline-none" placeholder="Contoh: Budi Santoso, S.H., M.H." />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <textarea rows={2} name="address" value={formData.address} onChange={handleChange} required
                  ...
                  <div>
                    ...
                    <div className="mt-1 relative rounded-md shadow-sm">
                      ...
                      <input type="tel" name="phone_number" value={formData.phone_number} onChange={handleChange} required
                        className="focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 sm:text-sm border-slate-300 rounded-md py-3 border outline-none" placeholder="08..." />
                    </div>
                  </div>
                  <div>
              ...
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select name="organization_name" value={formData.organization_name} onChange={handleChange} required
                      className="mt-1 block w-full pl-3 pr-10 py-3 text-base border-slate-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md border text-slate-700 bg-white">
                      <option value="">Pilih Organisasi</option>
                      <option value="PERADI">PERADI</option>
                      <option value="KAI">KAI</option>
                      <option value="AAI">AAI</option>
                      <option value="LBH Jakarta">LBH Jakarta</option>
                  ...
                  <div>
                    ...
                    <input type="text" name="license_number" value={formData.license_number} onChange={handleChange} required
                      className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-3 sm:text-sm border-slate-300 rounded-md py-3 border outline-none" placeholder="Contoh: PERADI-12345" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700">Pengalaman Hukum</label>
                  <textarea rows={2} name="experience" value={formData.experience} onChange={handleChange} required
                    className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-3 sm:text-sm border-slate-300 rounded-md py-3 border outline-none"
                    placeholder="Contoh: 5 tahun menangani kasus perdata dan pidana" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <textarea rows={2} name="office_address" value={formData.office_address} onChange={handleChange} required
                    className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-3 sm:text-sm border-slate-300 rounded-md py-3 border outline-none" placeholder="Alamat lengkap firma atau kantor hukum..." />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <input type="text" name="city" value={formData.city} onChange={handleChange} required
                      className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full px-3 sm:text-sm border-slate-300 rounded-md py-3 border outline-none" placeholder="Contoh: Jakarta Selatan" />
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {currentStep === 3 && (
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-800 font-medium">ℹ️ Upload dokumen verifikasi profesional Anda</p>
                  <p className="text-xs text-blue-600 mt-1">Admin akan memverifikasi keaslian dokumen sebelum akun Anda diaktifkan.</p>
                </div>
                <FileDropZone id="ktp-upload" label="Foto KTP" file={ktpFile} setFile={setKtpFile} hint="PNG, JPG, PDF maks. 5MB" />
                <FileDropZone id="license-upload" label="Kartu Tanda Pengenal Advokat (KTPA)" file={licenseFile}
                  setFile={setLicenseFile} icon={Award} hint="Bukti resmi keanggotaan advokat" />
                {statusMsg && (
