...
export default function AdvokatChatPage() {
  ...
  useEffect(() => {
    apiFetch("/cases/handled")
      .then((res) => {
        // The backend returns an array under 'data.cases'
  ...
  return (
    <div className="flex flex-col space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Kotak Masuk Pesan</h1>
        <p className="mt-1 text-sm text-slate-500">
          Pilih percakapan kasus untuk mulai berdiskusi dengan klien.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-xl border border-slate-200 text-center p-6">
          ...
          <h3 className="text-slate-900 font-semibold">Belum ada percakapan</h3>
          <p className="text-sm text-slate-500 max-w-xs mt-1">
            Percakapan akan muncul di sini setelah Anda mengambil kasus dari Klien.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {cases.map((c) => (
            <Link 
              key={c.id}
              href={`/advokat/kasus/${c.id}?tab=chat`}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900">{c.client_name || "Klien Anonim"}</h3>
